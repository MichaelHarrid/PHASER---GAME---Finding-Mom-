...
Scoreboard.prototype.show = function(score){
    ...
    bmd = this.game.add.bitmapData(this.game.width, this.game.height); //adding bitmapData
    bmd.ctx.fillStyle = '#000'; //black color
    bmd.ctx.fillRect(0,0, this.game.width, this.game.height); //draw rectangle

    background = this.game.add.sprite(0,0, bmd); //adding background
    background.alpha = 0.5; //opacity
    background.fixedToCamera = true; //enable to fixed with the camera
    ...
    gameoverText.fixedToCamera = true;//enable to fixed with the camera
    ...
    scoreText.fixedToCamera = true;//enable to fixed with the camera
    this.add(scoreText);
